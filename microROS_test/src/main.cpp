...
std_msgs__msg__Bool heartbeat_data;
...
{  
  RCLC_UNUSED(last_call_time);  // Prevent unused variable warning
  if (timer != NULL) {
    RCSOFTCHECK(rcl_publish(&heartbeat_publisher, &heartbeat_data, NULL));
  }
}

// Subscription callback function to be called on cmd_vel_relay recieved
...
void setup() {
  ...
  // Order added defines execution higherarchy (FIFO)
